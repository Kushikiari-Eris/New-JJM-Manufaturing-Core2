...
const useAuditRequestHr1Store = create((set) => ({
  requests: [],
  loading: false,
  ...
  fetchRequests: async () => {
    set({ loading: true });
    try {
      const response = await axios.get("/auditRequestHr1");
  ...
  addRequest: async (newRequest) => {
    try {
      const response = await axios.post("/auditRequestHr1", newRequest);
  ...
  deleteRequest: async (id) => {
    try {
      await axios.delete(`/auditRequestHr1/${id}`);
      set((state) => ({
...
export default useAuditRequestHr1Store;
