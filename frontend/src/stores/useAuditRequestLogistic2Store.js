...
const useAuditRequestLogistic2Store = create((set) => ({
  requests: [],
  loading: false,
  ...
  fetchRequests: async () => {
    set({ loading: true });
    try {
      const response = await axios.get("/auditRequestLogistic2");
  ...
  addRequest: async (newRequest) => {
    try {
      const response = await axios.post("/auditRequestLogistic2", newRequest);
  ...
  deleteRequest: async (id) => {
    try {
      await axios.delete(`/auditRequestLogistic2/${id}`);
      set((state) => ({
...
export default useAuditRequestLogistic2Store;
