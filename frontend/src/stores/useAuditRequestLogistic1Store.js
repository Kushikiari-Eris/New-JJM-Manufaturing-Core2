...
const useAuditRequestLogistic1Store = create((set) => ({
  ...
  fetchRequests: async () => {
    set({ loading: true });
    try {
      const response = await axios.get("/auditRequestLogistic1");
  ...
  deleteRequest: async (id) => {
    try {
      await axios.delete(`/auditRequestLogistic1/${id}`);
      set((state) => ({
...
export default useAuditRequestLogistic1Store;
