...
const useAuditRequestCore1Store = create((set) => ({
  requests: [],
  loading: false,
  ...
  fetchRequests: async () => {
    set({ loading: true });
    try {
      const response = await axios.get("/auditRequestCore1");
  ...
  addRequest: async (newRequest) => {
    try {
      const response = await axios.post("/auditRequestCore1", newRequest);
  ...
  deleteRequest: async (id) => {
    try {
      await axios.delete(`/auditRequestCore1/${id}`);
      set((state) => ({
...
export default useAuditRequestCore1Store;
