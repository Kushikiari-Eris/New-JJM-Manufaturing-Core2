...
const UserComponents = () => {
    ...
    const totalPages = Math.ceil(userArray.length / itemsPerPage) || 1;

    // Paginate only when userArray or currentPage changes
    useEffect(() => {
        if (userArray.length === 0) {
            setPaginatedRequests([]); // Prevent unnecessary calculations
            return;
        }
        const indexOfLastItem = currentPage * itemsPerPage;
        const indexOfFirstItem = indexOfLastItem - itemsPerPage;
        setPaginatedRequests(userArray.slice(indexOfFirstItem, indexOfLastItem));
    }, [user, currentPage, itemsPerPage]); // **Do not depend on userArray**

    // Ensure pagination doesn't go out of bounds when userArray updates
    useEffect(() => {
