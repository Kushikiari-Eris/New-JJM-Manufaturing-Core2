...
export const getCompletedTasks = async (req, res) => {
  try {
    const tasks = await CompletedTasksLogistic2.find();
...
export const getCompletedTaskById = async (req, res) => {
  try {
    const task = await CompletedTasksLogistic2.findById(req.params.id);
...
export const addCompletedTask = async (req, res) => {
  try {
    const newTask = new CompletedTasksLogistic2(req.body);
    await newTask.save();
...
export const deleteCompletedTask = async (req, res) => {
  try {
    await CompletedTasksLogistic2.findByIdAndDelete(req.params.id);
