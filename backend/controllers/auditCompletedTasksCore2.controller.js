...
export const getCompletedTasks = async (req, res) => {
  try {
    const tasks = await CompletedTasksCore2.find();
...
export const getCompletedTaskById = async (req, res) => {
  try {
    const task = await CompletedTasksCore2.findById(req.params.id);
...
export const addCompletedTask = async (req, res) => {
  try {
    const newTask = new CompletedTasksCore2(req.body);
    await newTask.save();
...
export const deleteCompletedTask = async (req, res) => {
  try {
    await CompletedTasksCore2.findByIdAndDelete(req.params.id);
