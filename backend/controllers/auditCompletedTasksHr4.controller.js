...
export const getCompletedTasks = async (req, res) => {
  try {
    const tasks = await CompletedTasksHR4.find();
...
export const getCompletedTaskById = async (req, res) => {
  try {
    const task = await CompletedTasksHR4.findById(req.params.id);
...
export const addCompletedTask = async (req, res) => {
  try {
    const newTask = new CompletedTasksHR4(req.body);
    await newTask.save();
...
export const deleteCompletedTask = async (req, res) => {
  try {
    await CompletedTasksHR4.findByIdAndDelete(req.params.id);
