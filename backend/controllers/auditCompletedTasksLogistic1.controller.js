...
export const getCompletedTasks = async (req, res) => {
  try {
    const tasks = await CompletedTasksLogistic1.find();
...
export const getCompletedTaskById = async (req, res) => {
  try {
    const task = await CompletedTasksLogistic1.findById(req.params.id);
...
export const addCompletedTask = async (req, res) => {
  try {
    const newTask = new CompletedTasksLogistic1(req.body);
    await newTask.save();
...
export const deleteCompletedTask = async (req, res) => {
  try {
    await CompletedTasksLogistic1.findByIdAndDelete(req.params.id);
