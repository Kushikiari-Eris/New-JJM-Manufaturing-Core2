...
export const getCompletedTasks = async (req, res) => {
  try {
    const tasks = await CompletedTasksFinance.find();
...
export const getCompletedTaskById = async (req, res) => {
  try {
    const task = await CompletedTasksFinance.findById(req.params.id);
...
export const addCompletedTask = async (req, res) => {
  try {
    const newTask = new CompletedTasksFinance(req.body);
    await newTask.save();
...
export const deleteCompletedTask = async (req, res) => {
  try {
    await CompletedTasksFinance.findByIdAndDelete(req.params.id);
